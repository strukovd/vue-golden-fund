...
<template>
	<div class="modal-shadow" @keydown.esc="close" @click.self="close">
		...
		<main class="main">
			<component
				v-if="visibleModalComponent"
				:is="visibleModalComponent"
				:payload="visibleModal.options?.payload"
			/>
		</main>
...
export default {
	computed: {
		...
		visibleModalComponent() {
			return resolveModalComponent(this.visibleModal?.template);
		}
	},
	methods: {
